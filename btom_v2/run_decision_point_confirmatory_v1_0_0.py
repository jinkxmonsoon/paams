...
from .analyze_decision_point_confirmatory_v1_0_0 import analyze,subgroup_results
...
def role_pair_diagnostics(records):
 by={(r['family'],r['variant_id'],r['state'],r['condition']):r for r in records};pairs=[]
 for family,reference,belief in (('H1','matched_decision_record','explicit_self_belief'),('H2','self_belief_plus_message_record','self_belief_plus_partner_belief')):
  for variant in sorted({r['variant_id'] for r in records if r['family']==family}):
   for state in ('stale','current'):
    ref,bel=by.get((family,variant,state,reference)),by.get((family,variant,state,belief))
    if not ref or not bel:continue
    complete=ref['complete'] and bel['complete'];same_action=(ref['action'],ref['target'])==(bel['action'],bel['target']) if complete else None
    pairs.append({'family':family,'variant_id':variant,'state':state,'reference_prompt_id':ref['prompt_id'],'belief_prompt_id':bel['prompt_id'],'same_requested_seed':ref['requested_seed']==bel['requested_seed'],'reference_returned_seed_matches':ref['seed_matches'],'belief_returned_seed_matches':bel['seed_matches'],'same_system_fingerprint':ref['system_fingerprint']==bel['system_fingerprint'],'same_service_tier':ref['service_tier']==bel['service_tier'],'technically_complete':complete,'reference_action_target':[ref['action'],ref['target']] if ref['legal_action'] else None,'belief_action_target':[bel['action'],bel['target']] if bel['legal_action'] else None,'same_action':same_action,'different_action':not same_action if complete else None,'reference_classification':ref['classification'],'belief_classification':bel['classification']})
 return {'pair_count':len(pairs),'complete_pairs':sum(p['technically_complete'] for p in pairs),'fingerprint_concordant_pairs':sum(p['same_system_fingerprint'] for p in pairs),'pairs':pairs}
def technical_coverage(records):
 dimensions=('family','condition','state','archetype','difficulty');return {'complete_calls':sum(r['complete'] for r in records),'total_calls':len(records),'by_dimension':{d:[{'value':v,'calls':len(rows),'complete_calls':sum(r['complete'] for r in rows)} for v in sorted({r[d] for r in records}) for rows in [[r for r in records if r[d]==v]]] for d in dimensions}}
def execute(out:Path,sleep=time.sleep,client_factory=Client):
 ...
 try:
  ...
  jsonl(out/'call_records.jsonl',records);jsonl(out/'raw_api_responses.jsonl',[{'ordinal':r['ordinal'],'raw_api_response':r['raw_api_response']} for r in records]);jsonl(out/'behavioral_results.jsonl',[{k:r[k] for k in ('ordinal','family','variant_id','state','condition','complete','action','target','classification','fallback_counted_as_behavior')} for r in records]);analysis=analyze(records) if len(records)==432 else {'inferential_unit':'variant','variant_results':[],'role_sensitivity':{},'mcnemar_role_tests':{},'H1a':{}};pairs=role_pair_diagnostics(records);dump(out/'variant_level_results.json',analysis['variant_results']);dump(out/'role_sensitivity_confidence_bounds.json',analysis['role_sensitivity']);dump(out/'role_pair_diagnostics.json',pairs);dump(out/'technical_coverage.json',technical_coverage(records));dump(out/'archetype_results.json',subgroup_results(analysis['variant_results'],'archetype','surface_domain_template'));dump(out/'difficulty_results.json',subgroup_results(analysis['variant_results'],'difficulty','surface_perturbation_stratum'));dump(out/'mcnemar_results.json',{'role_tests':analysis.get('mcnemar_role_tests',{}),'H1a':analysis.get('H1a',{})});dump(out/'seed_and_fingerprint_diagnostics.json',{'records':[{'prompt_id':r['prompt_id'],'requested_seed':r['requested_seed'],'returned_seed':r['returned_seed'],'seed_matches':r['seed_matches'],'system_fingerprint':r['system_fingerprint'],'service_tier':r['service_tier']} for r in records]});dump(out/'usage_and_latency.json',{'records':[{'ordinal':r['ordinal'],'usage':r['usage'],'latency':r['latency']} for r in records]});dump(out/'execution_environment.json',{'python':sys.version,'platform':platform.platform(),'versions':versions,'authorization_value_recorded':False});classification='transport_failure' if transport else ('experiment_complete' if len(records)==432 and any(r['http_success'] for r in records) else 'runtime_failure');dump(out/'confirmatory_summary.json',{'operational_classification':classification,'H1_primary_interpretable':analysis.get('role_sensitivity',{}).get('H1',{}).get('complete_variants')==36,'H2_primary_interpretable':analysis.get('role_sensitivity',{}).get('H2',{}).get('complete_variants')==36,'complete_calls':sum(r['complete'] for r in records),'complete_variants':{f:analysis.get('role_sensitivity',{}).get(f,{}).get('complete_variants',0) for f in ('H1','H2')},'role_sensitive_counts':{f:analysis.get('role_sensitivity',{}).get(f,{}).get('role_sensitive_variants',0) for f in ('H1','H2')},'invariance_criteria':analysis.get('role_sensitivity',{}),'H1a':analysis.get('H1a',{}),'role_mcnemar_results':analysis.get('mcnemar_role_tests',{}),'replication_not_executed':'openai/gpt-oss-120b','scientific_inference':None});return 0 if classification=='experiment_complete' else 1
