"""Execute the frozen fresh full dual-account confirmatory batch exactly once."""
...
from .analyze_decision_point_confirmatory_v1_0_0 import analyze
ROOT=Path(__file__).resolve().parents[1]
MANIFEST=Path(__file__).with_name('decision_point_confirmatory_full_dual_account_manifest_v1_0_0.json')
COLLECTION_BATCH='full_confirmatory_20b_dual_account_fresh'; MAX_REQUESTS=432; DELAY_SECONDS=20; RETRIES=0
OUTPUTS=('confirmatory_full_dual_summary.json','confirmatory_full_dual_manifest_snapshot.json','confirmatory_full_dual_selected_scenarios.jsonl','confirmatory_full_dual_selected_prompts.jsonl','confirmatory_full_dual_request_order.json','confirmatory_full_dual_prompt_audit.json','confirmatory_full_dual_token_pair_audit.json','confirmatory_full_dual_call_records.jsonl','confirmatory_full_dual_raw_api_responses.jsonl','confirmatory_full_dual_behavioral_results.jsonl','confirmatory_full_dual_technical_coverage.json','confirmatory_full_dual_variant_completeness.json','confirmatory_full_dual_seed_and_fingerprint_diagnostics.json','confirmatory_full_dual_usage_and_latency.json','confirmatory_full_dual_immutable_input_hashes.json','confirmatory_full_dual_execution_environment.json','confirmatory_full_dual_assignment.json','confirmatory_full_dual_credential_routing_summary.json','confirmatory_full_dual_transport_attempts.jsonl','confirmatory_full_dual_discarded_transport_attempts.jsonl','confirmatory_full_dual_tpd_failover_diagnostics.json')
...
def selected_prompts():return request_order()
...
def assignment_audit(prompts,assignment,order=None):
 ...
 triplets={(r['family'],r['variant_id'],r['state']):r for r in order['triplets']};slots={}
 for slot in ('primary','secondary'):
  variants=[v for v in sorted(by_variant) if assignment.get(v)==slot];strata={}
  for family in ('H1','H2'):
   for difficulty in ('direct','irrelevant_distractor','compositional'):
    vs=[v for v in variants if by_variant[v][0].family==family and by_variant[v][0].difficulty==difficulty]
    patterns=Counter();role=Counter();reactive=Counter();positions={'record':Counter(),'belief':Counter()}
    for v in vs:
     for state in ('stale','current'):
      pattern=triplets[(family,v,state)]['pattern'];roles=ORDER_PATTERNS[pattern];patterns[pattern]+=1;role[roles[1] if roles[0]=='reactive' else roles[0]]+=1;reactive['before' if roles[0]=='reactive' else 'after']+=1
      for name in ('record','belief'):positions[name][str(roles.index(name)+1)]+=1
    strata[f'{family}:{difficulty}']={'variants':len(vs),'archetypes':dict(Counter(by_variant[v][0].archetype for v in vs)),'order_patterns':dict(patterns),'role_first':dict(role),'reactive_placement':dict(reactive),'record_positions':dict(positions['record']),'belief_positions':dict(positions['belief'])}
  slots[slot]={'variants':len(variants),'families':dict(Counter(by_variant[v][0].family for v in variants)),'difficulties':dict(Counter(by_variant[v][0].difficulty for v in variants)),'strata':strata}
 expected={'variants':6,'order_patterns':{'A':3,'B':3,'C':3,'D':3},'role_first':{'record':6,'belief':6},'reactive_placement':{'before':6,'after':6},'record_positions':{'1':3,'2':6,'3':3},'belief_positions':{'1':3,'2':6,'3':3}}
 split=all({assignment[v] for v in by_variant if by_variant[v][0].family==f and by_variant[v][0].difficulty==d and by_variant[v][0].archetype==a}=={'primary','secondary'} for f in ('H1','H2') for d in ('direct','irrelevant_distractor','compositional') for a in {by_variant[v][0].archetype for v in by_variant if by_variant[v][0].family==f})
 passed=set(assignment)==set(by_variant) and split and all(row['variants']==36 and row['families']=={'H1':18,'H2':18} and row['difficulties']=={'direct':12,'irrelevant_distractor':12,'compositional':12} and all(all(s[k]==v for k,v in expected.items()) and set(s['archetypes'].values())=={1} for s in row['strata'].values()) for row in slots.values())
 return {'passed':passed,'archetype_pairs_split':split,'slots':slots}
def preclient_audit(manifest,full,prompts,token_pairs,client_constructed=False):
 ...
 rows=[{'confirmatory_full_dual_ordinal':i,'original_ordinal':original_ordinals[p.prompt_id],'prompt_id':p.prompt_id,'family':p.family,'variant_id':p.variant_id,'archetype':p.archetype,'difficulty':p.difficulty,'state':p.state,'condition':p.condition,'seed':p.seed,'prompt_sha256':hashlib.sha256(p.prompt.encode()).hexdigest(),'valid_actions':[list(x) for x in p.valid_actions]} for i,p in enumerate(prompts,1)]
 checks={'client_not_constructed':not client_constructed,'full_bank_prompt_count':len(full)==432,'full_bank_canonical_digest':canonical_prompt_digest(full)==manifest['prompt_freeze']['full_bank_canonical_sha256'],'scenario_bank_hash':hashlib.sha256((ROOT/'btom_v2/decision_point_confirmatory_scenarios_v1_0_0.py').read_bytes()).hexdigest()==manifest['prompt_freeze']['scenario_bank_sha256'],'selected_prompt_count':len(prompts)==432,'unique_selected_prompt_ids':len({p.prompt_id for p in prompts})==432,'selected_variants':{f:len(v) for f,v in variants.items()}=={'H1':36,'H2':36},'all_frozen_difficulties':{p.difficulty for p in prompts}=={'direct','irrelevant_distractor','compositional'},'manifest_selected_order_identity':rows==selected_manifest,'selected_ids_subset_full_bank':all(p.prompt_id in full_by_id for p in prompts),'prompt_text_seed_identity':all(p.prompt==full_by_id[p.prompt_id].prompt and p.seed==full_by_id[p.prompt_id].seed and p.valid_actions==full_by_id[p.prompt_id].valid_actions for p in prompts),'relative_ordinals_strictly_increasing':all(a<b for a,b in zip([original_ordinals[p.prompt_id] for p in prompts],[original_ordinals[p.prompt_id] for p in prompts][1:])),'triplet_count':len(order['triplets'])==144,'unique_triplet_seeds':len({r['seed'] for r in order['triplets']})==144,'same_seed_in_triplets':all(r['same_seed'] for r in order['triplets']),'global_pattern_balance':order['global_pattern_counts']=={'A':36,'B':36,'C':36,'D':36},'family_state_balance':all(r['pattern_counts']=={'A':9,'B':9,'C':9,'D':9} and r['role_first']=={'record':18,'belief':18} and r['reactive_placement']=={'before':18,'after':18} and r['positions']=={'record':{1:9,2:18,3:9},'belief':{1:9,2:18,3:9}} for r in order['family_state_strata'].values()),'role_pair_adjacency':all(r['role_pair_adjacent'] for r in order['triplets']),'raw_token_parity':token_pairs['pair_count']==144 and token_pairs['all_raw_parity'],'harmony_token_parity':token_pairs['pair_count']==144 and token_pairs['all_harmony_parity'],'critical_pairs_content_matched':token_pairs['all_content_matched'],'no_development_prompt_overlap':semantic['prompt_overlap_count']==0,'no_development_entity_overlap':semantic['entity_overlap_count']==0,'no_state_or_scoring_leakage':semantic['model_visible_state_or_scoring_leaks']==0,'request_body_identity':all(body(p)==body(full_by_id[p.prompt_id]) for p in prompts)}
...
def execute(out:Path,sleep=time.sleep,client_factory=default_client_factory,environ=None):
 ...
 try:
  manifest=json.loads(MANIFEST.read_text());dump(out/'confirmatory_full_dual_manifest_snapshot.json',manifest);dump(out/'confirmatory_full_dual_immutable_input_hashes.json',immutable_hashes(manifest));full=request_order();prompts=selected_prompts();raw,harmony,versions=load_tokenizers(manifest);tokens=token_pair_audit(prompts,raw,harmony);audit=preclient_audit(manifest,full,prompts,tokens,client_constructed=False);credentials=load_credentials(environ);audit['checks']['dual_credentials_present']=True;audit['checks']['credentials_distinct']=True;audit['passed']=all(audit['checks'].values());dump(out/'confirmatory_full_dual_prompt_audit.json',audit);dump(out/'confirmatory_full_dual_token_pair_audit.json',tokens)
  if not audit['passed']:raise RuntimeError('pre-client full dual audit failed')
  assignment=variant_assignment(manifest);dump(out/'confirmatory_full_dual_assignment.json',{'collection_batch':COLLECTION_BATCH,'variant_assignment':assignment,'audit':audit['credential_assignment_audit'],'assignment_independent_of_prior_outputs':True});all_secret_values=tuple(credentials.values());clients={slot:client_factory(slot,key,all_secret_values) for slot,key in credentials.items()};clients_constructed=True;original_ordinals={p.prompt_id:i for i,p in enumerate(full,1)};by_variant=defaultdict(list)
  for p in prompts:by_variant[p.variant_id].append(p)
  jsonl(out/'confirmatory_full_dual_selected_scenarios.jsonl',[r for r in bank_records() if r['variant_id'] in by_variant]);jsonl(out/'confirmatory_full_dual_selected_prompts.jsonl',[{'prompt_id':p.prompt_id,'prompt':p.prompt,'prompt_sha256':hashlib.sha256(p.prompt.encode()).hexdigest(),'seed':p.seed,'valid_actions':p.valid_actions} for p in prompts]);dump(out/'confirmatory_full_dual_request_order.json',audit['selected_manifest_rows'])
  def attempt(p,slot):
  ...
  for i,r in enumerate(records,1):r['confirmatory_full_dual_ordinal']=i
  jsonl(out/'confirmatory_full_dual_transport_attempts.jsonl',attempts);jsonl(out/'confirmatory_full_dual_discarded_transport_attempts.jsonl',discarded);jsonl(out/'confirmatory_full_dual_call_records.jsonl',records);jsonl(out/'confirmatory_full_dual_raw_api_responses.jsonl',[{'confirmatory_full_dual_ordinal':r['confirmatory_full_dual_ordinal'],'prompt_id':r['prompt_id'],'collection_batch':COLLECTION_BATCH,'raw_api_response':r['raw_api_response']} for r in records]);jsonl(out/'confirmatory_full_dual_behavioral_results.jsonl',[{k:r[k] for k in ('confirmatory_full_dual_ordinal','original_full_bank_ordinal','prompt_id','family','variant_id','archetype','difficulty','state','condition','collection_batch','assigned_credential_slot','final_credential_slot','credential_failover_used','complete','action','target','classification','fallback_counted_as_behavior')} for r in records]);variants,technical=coverage(records);dump(out/'confirmatory_full_dual_technical_coverage.json',technical);dump(out/'confirmatory_full_dual_variant_completeness.json',variants);dump(out/'confirmatory_full_dual_seed_and_fingerprint_diagnostics.json',{'records':[{k:r[k] for k in ('prompt_id','requested_seed','returned_seed','seed_matches','system_fingerprint','service_tier')} for r in records]});total_usage={k:sum((r['usage'] or {}).get(k,0) for r in records) for k in ('prompt_tokens','completion_tokens','total_tokens')};dump(out/'confirmatory_full_dual_usage_and_latency.json',{'records':[{'prompt_id':r['prompt_id'],'usage':r['usage'],'latency':r['latency']} for r in records],'total_usage':total_usage});dump(out/'confirmatory_full_dual_execution_environment.json',{'python':sys.version,'platform':platform.platform(),'dependencies':versions,'collection_batch':COLLECTION_BATCH,'dual_credentials_present':True,'credentials_distinct':True,'authorization_values_recorded':False})
  final_slots={v:{r['final_credential_slot'] for r in records if r['variant_id']==v} for v in by_variant};homogeneous=all(len(slots)==1 for slots in final_slots.values()) and len(final_slots)==72;variants_by_slot=Counter(next(iter(slots)) for slots in final_slots.values() if len(slots)==1);routing={'variants_by_final_credential_slot':dict(variants_by_slot),'all_variants_credential_homogeneous':homogeneous,'failover_variants':sorted(failover_variants),'exhausted_credential_slots':sorted(exhausted)};dump(out/'confirmatory_full_dual_credential_routing_summary.json',routing);dump(out/'confirmatory_full_dual_tpd_failover_diagnostics.json',{'failover_count':len(failover_variants),'failover_trigger_ordinal':failover_trigger,'exhausted_credential_slot':next(iter(exhausted),None) if len(exhausted)==1 else None,'dual_account_tpd_exhausted':dual_exhausted,'discarded_transport_attempt_count':len(discarded)})
  seed_ok=all(r['seed_matches'] is not False for r in records);unique_ids=len({r['prompt_id'] for r in records})==432
  ready=len(records)==432 and sum(r['complete'] for r in records)==432 and technical['complete_variants_by_family']=={'H1':36,'H2':36} and homogeneous and unique_ids and seed_ok and audit['passed'] and not dual_exhausted
  if ready:
   analysis=analyze(records);by_slot={}
   for slot in ('primary','secondary'):
    vr=[v for v in analysis['variant_results'] if next(r for r in records if r['variant_id']==v['variant_id'])['final_credential_slot']==slot]
    by_slot[slot]={'final_variants':len(vr),'family_counts':dict(Counter(v['family'] for v in vr)),'difficulty_counts':dict(Counter(v['difficulty'] for v in vr)),'role_sensitive_by_family':{f:sum(v['role_sensitive_variant'] for v in vr if v['family']==f) for f in ('H1','H2')},'H1a_task_pass_by_condition':{c:sum(v['condition_metrics'][c]['task_passed'] for v in vr if v['family']=='H1') for c in H1_CONDITIONS}}
   analysis['credential_slot_diagnostic']={'descriptive_only':True,'no_credential_slot_significance_testing':True,'not_used_for_exclusion':True,'not_used_for_primary_inference':True,'slots':by_slot}
   dump(out/'confirmatory_full_dual_analysis.json',analysis)
  operational='full_confirmatory_batch_complete' if ready else ('dual_account_tpd_exhausted' if dual_exhausted else ('transport_failure' if transport_failure else 'full_confirmatory_batch_incomplete'));summary={'operational_classification':operational,'source_commit':manifest['source_commit'],'collection_batch':COLLECTION_BATCH,'canonical_prompt_count':len(records),'transport_attempt_count':len(attempts),'canonical_complete_calls':sum(r['complete'] for r in records),'complete_variants_by_family':technical['complete_variants_by_family'],'variants_by_final_credential_slot':dict(variants_by_slot),'failover_count':len(failover_variants),'exhausted_credential_slots':sorted(exhausted),'dual_account_tpd_exhausted':dual_exhausted,'all_variants_credential_homogeneous':homogeneous,'previous_confirmatory_observations_excluded':True,'ready_for_final_analysis':ready,'scientific_inference':None,'total_usage':total_usage};dump(out/'confirmatory_full_dual_summary.json',summary);return 0 if ready else 1
 except Exception as exc:
  dump(out/'confirmatory_full_dual_summary.json',{'operational_classification':'runtime_failure','collection_batch':COLLECTION_BATCH,'canonical_prompt_count':len(final_records),'transport_attempt_count':len(attempts),'error':sanitize(exc,credentials.values()),'previous_confirmatory_observations_excluded':True,'ready_for_final_analysis':False,'scientific_inference':None,'final_confirmatory_analysis_not_performed':True});return 1
def main():
