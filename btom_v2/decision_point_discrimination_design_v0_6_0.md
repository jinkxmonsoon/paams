...
Run 30384654514 returned 48/48 HTTP 403 responses with Cloudflare error 1010 and no model output. The transport now sends an explicit browser-compatible project User-Agent and JSON Accept header, and fails fast on a pre-inference 1010 response. This changes request headers only: prompts, scenarios, scoring, selection, request body, and all scientific inputs remain frozen. It is a pre-inference operational repair, not a post-outcome protocol change.
