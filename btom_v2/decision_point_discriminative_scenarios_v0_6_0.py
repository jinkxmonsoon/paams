...
for i,((room_a,room_b),(route_a,route_b)) in enumerate(zip(ROOMS,ROUTES),1):
 current=room_a if i<=2 else room_b
 stale=room_b if current==room_a else room_a
 actions=(('move',route_a),('move',route_b))
 mapping={room_a:route_a,room_b:route_b}
 for state,represented in (('stale',stale),('current',current)):
  ...
  H1_SCENARIOS.append(Scenario('H1',f'H1V{i}',state,room_a,room_b,route_a,route_b,represented,current,'','',actions,classes))
 for state in STATES:
  actions2=(('send_message','partner'),('progress','resource'))
  classes2=((actions2[0],'necessary_correction' if state=='stale' else 'unnecessary_correction'),(actions2[1],'missed_necessary_correction' if state=='stale' else 'appropriate_progress'))
  H2_SCENARIOS.append(Scenario('H2',f'H2V{i}',state,'','','','',stale if state=='stale' else current,current,f'partner {i}',f'resource {i}',actions2,classes2))
SCENARIOS=tuple(H1_SCENARIOS+H2_SCENARIOS)
SCENARIO_BY_KEY={(s.family,s.variant,s.state):s for s in SCENARIOS}
