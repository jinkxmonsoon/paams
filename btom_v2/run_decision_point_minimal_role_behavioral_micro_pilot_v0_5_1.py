...
def request_body(prompt,case):
 return {'model':MODEL,'messages':[{'role':'user','content':prompt}],'temperature':0,'top_p':1,'max_completion_tokens':1024,'reasoning_effort':'low','include_reasoning':False,'response_format':response_schema(case),'stream':False}
...
def pair_diagnostics(records):
 ...
 for h,case,reference,treatment in PRIMARY_PAIRS:
  ref,tr=by[(case,reference)],by[(case,treatment)]; ready=all(r['http_success'] and not r['empty_visible_content'] and r['completed_finish_reason'] and r['parse_success'] and r['legal_action'] for r in (ref,tr))
