...
def analyze(records):
 ...
 for family,reference,belief in configs:
  for variant in sorted({r['variant_id'] for r in records if r['family']==family}):
   sample=next(r for r in records if r['family']==family and r['variant_id']==variant);conditions=('reactive_no_representation',reference,belief);metrics={c:condition_metrics(family,by.get((family,variant,'stale',c)),by.get((family,variant,'current',c))) for c in conditions};role_complete=metrics[reference]['technically_complete'] and metrics[belief]['technically_complete'];sensitive=role_complete and any((by[(family,variant,s,reference)]['action'],by[(family,variant,s,reference)]['target'])!=(by[(family,variant,s,belief)]['action'],by[(family,variant,s,belief)]['target']) for s in ('stale','current'));fingerprints=all(by[(family,variant,state,reference)]['system_fingerprint'] is not None and by[(family,variant,state,belief)]['system_fingerprint'] is not None and by[(family,variant,state,reference)]['system_fingerprint']==by[(family,variant,state,belief)]['system_fingerprint'] for state in ('stale','current')) if role_complete else False
